...
def generate_password():
    adjectives = ['crazy', 'sunny', 'happy', 'wild', 'quick', 'witty', 'jolly', 'zany', 'lazy']
    nouns = ['cat', 'evening', 'river', 'breeze', 'mountain', 'ocean', 'sun', 'moon', 'tree']
    password = random.choice(adjectives) + random.choice(nouns) + ''.join(random.choices(string.digits, k=4))
    return password

# Function to execute shell commands to create a user
def create_system_user(username, password):
    try:
        hashed_password = subprocess.run(['openssl', 'passwd', '-6', password], check=True, capture_output=True, text=True).stdout.strip()
        subprocess.run(['sudo', 'useradd', '-m', '-p', hashed_password, username], check=True)
...
async def create_user(event):
    ...
    if len(args) < 3:
        await event.respond("❓ Usage: /create_user <username> <plan_duration> \nFor example: `/create_user john 7d`")
        return
    ...
    await event.respond(f"✨ User `{username}` created with password `{password}`.\n📅 Plan expires on `{expiry_date}`.")
...
async def extend_plan(event):
    args = event.message.text.split()
    if len(args) < 3:
        await event.respond("❓ Usage: /extend_plan <username> <additional_duration>\nFor example: `/extend_plan john 5d`")
        return
...
async def extend_plan_helper(event, username, additional_seconds):
    ...
    if result:
        ...
        new_expiry_date = datetime.fromtimestamp(new_expiry_time).strftime('%Y-%m-%d %H:%M:%S')
        await event.respond(f"🔄 User `{username}`'s plan extended to `{new_expiry_date}`.")
    else:
        await event.respond(f"❌ User `{username}` not found.")
...
async def notify_expiry():
    while True:
        now = int(time.time())
        cursor.execute('SELECT user_id, username FROM users WHERE expiry_time<=?', (now,))
        expired_users = cursor.fetchall()

        for user_id, username in expired_users:
            await client.send_message(user_id, f"⏰ Your plan has expired, `{username}`! Please renew soon. 😊")
            # subprocess.run(['sudo', 'userdel', '-r', username], check=True)
            # cursor.execute('DELETE FROM users WHERE username=?', (username,))
            # conn.commit()
